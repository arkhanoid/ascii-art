...
#endif
/*
 ___$_____*_____0123456789______
_xBxxEFxxxxKxxxxPxxSTxVxXYZ_____@
_abxdexghxxxxxnopqxxxuvxxyz____
____$

*/
#define PROIBIDOS " !\"&#%$'()+,-./:;<=>?@ACDGHIJLMNOQRUWcfijklmrstw[\\]^{|}~`"
...
initialize_font (int *p1, int *p2)
{
  ...
  font_y = f1.H / 3;
#if DEBUG
  fprintf (debugfd, "Detected FX=%d, FY=%d\n", font_x, font_y);
#endif
...
#ifdef DEBUG
          fprintf (debugfd, "\nMAPA de [%c]\n", isprint (i) ? i : '.');
          for (j = 0; j < FONT_H; j++, putc (10, debugfd))
	    for (k = 0; k < FONT_W; k++)
	      {
		aux = IS_BIT (i, k, j);
		if (aux < 0.25)
