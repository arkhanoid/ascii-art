...
main (int argc, char **argv) 
{
  ...
  double np, op, err;
  unsigned int *image;
  unsigned int pal[ncolors] = { 255, 238, 224, 220, 212, 208, 204, 195, 181, 176, 155, 136, 121, 113, 94, 69, 0 };	/* lucida console, on notepad */
...
//     PIX(x, y) =  gamma((30 * r + 59 * g + 11 * b) / 100, GAMMA);
	    PIX (x, y) =
	    (30 * gamma (r, GAMMA) + 59 * gamma (g, GAMMA) + 11 * gamma (b, GAMMA)) / 100;
	}
      for (i = 0; i < pad; i++)
	fgetc (fpi);
    }
